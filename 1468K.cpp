...
pi block(string s, pi p) {
    int x = 0, y = 0;
    for(auto i : s) {
        if(i == 'R') {
            if(p.second == y && p.first == x + 1) continue;
            x++;
        }
        else if(i == 'L') {
            if(p.second == y && p.first == x - 1) continue;
            ...
            if(p.first == x && p.second == y + 1) continue;
            y++;
        }
        else {
            if(p.first == x && p.second == y - 1) continue;
...
pi rec(string s) {
    ...
    for(auto i : s) {
        if(i == 'R') {
            // block 
            pi p = block(s, {x+1, y});
            if(p.first == 0 && p.second == 0) return {x+1, y};
            // continue
            x++;
        }
        else if(i == 'L') {
            ...
            // block 
            pi p = block(s, {x, y+1});
            if(p.first == 0 && p.second == 0) return {x, y+1};
            // continue
            y++;
        }
        else {
