...
int main() {
    ...
    while (t--) {
        ...
        for(int i = q-1; i >= 0 ; i--){
            auto p = v[i];
            bool flag = false;
            if(rows.find(p.first) == rows.end()){
                rows.insert(p.first); flag = true;
            }
            if(cols.find(p.second) == cols.end()){
                cols.insert(p.second); flag = true;
            }
            if(flag) ans++;
            if(rows.size() == n || cols.size() == m) break;
        }
        debug(ans);
        cout<<power(k, ans, 998244353)<<endl;
    }
    return 0;
}