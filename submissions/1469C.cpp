...
int main() {
    ...
    while(t--) {
        ...
        for(int i = 0; i < n ; i++){
            int x; cin>>x;
            h[i] = x;
            if(i == 0 || i == n-1) v[i] = {x, x + k};
            else {
                int first = max(x, v[i-1].first + 1 - k);
                int second = min(x + k - 1 + k, v[i-1].second - 1 + k);
                v[i] = {first, second};
            }
        }
        bool ok = true;
        for(int i = 0; i < n ; i++){
            if(v[i].first > h[i] + k - 1) ok = false;
            if(i > 0) {
                if(v[i].first > v[i-1].second - 1) ok = false;
                if(v[i].second < v[i-1].first + 1) ok = false;
            }
        }
        cout<<(ok ? "YES" : "NO" ) <<endl;
    }
