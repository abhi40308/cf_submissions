...
void dfs(int root, int p, vector<vector<ll> > &dp) {
    // debug(root);
    for(auto i : v[root]) {
        if(i == p) continue;
        // debug(i);
        dfs(i, root, dp);
        dp[root][0] += max(dp[i][0] + abs(l[i] - l[root]), dp[i][1] + abs(r[i] - l[root]));
        dp[root][1] += max(dp[i][0] + abs(l[i] - r[root]), dp[i][1] + abs(r[i] - r[root]));
    }
}
...
int main() {
    ...
    while(t--) {
        ...
        int leaf;
        for(int i = 1; i <= n ; i++){
            if(v[i].size() == 1) {leaf= i; break;}
        }
        // debug(leaf);
        vector<vector<ll> > dp(n + 1, vector<ll> (2, 0));
        dfs(leaf, -1, dp);
        cout<<max(dp[leaf][0], dp[leaf][1])<<endl;
    }
    return 0;
}